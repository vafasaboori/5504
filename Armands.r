...
armands %>% ggplot(aes(x = Population, y = residuals)) + 
  ...
  geom_smooth() # we observe no pattern
...
# QQ Plot (method 1)
qplot(sample = armands$std_residuals)
...
plot(armands_slr, which = 1) # produces 5 plots
...
plot(armands_slr, which = 4) #none greater than 4/10=0.4
...
plot(armands_slr, which = 5) #none greater than 4/10=0.4
